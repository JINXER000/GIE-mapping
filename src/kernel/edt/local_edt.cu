...
namespace EDT_OCC
{
    void batchEDTUpdate(LocMap *loc_map, cuttHandle *plan, const int time)
    {
        ...
        //Phase 1
        EDT_CORE::EDTphase1<<<loc_map->_update_size.z,loc_map->_update_size.x>>>(*loc_map,time);
        cuttExecute(plan[0], loc_map->_g, loc_map->_aux);

        //Phase 2
        EDT_CORE::EDTphase2<<<loc_map->_update_size.z,loc_map->_update_size.y>>>(*loc_map);
        cuttExecute(plan[1], loc_map->_g, loc_map->_aux);

        //Phase 3 (needed only for 3D map)
        if (loc_map->_update_size.z > 1)
        {
            EDT_CORE::EDTphase3<<<loc_map->_update_size.x,loc_map->_update_size.y>>>(*loc_map);
